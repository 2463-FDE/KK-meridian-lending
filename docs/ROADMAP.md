...
**How the evidence was obtained, including what was not run.** Every backend
suite was executed locally (gateway 98, kyc 90, origination 213, decision 40,
payment 158, loan-assistant 238, disclosure 144, servicing 263, `db/tests` 305 —
all passing). **No PostgreSQL was available**, so every real-database case
skipped: 328 in `db/tests`, 87 in origination, 32 in disclosure, 29 in servicing,
13 in payment. A skip prints as a dot-adjacent `s` and reads like a pass, which
is why the rows that depend on one say so in their own words. The executed
database evidence for this commit is CI, not this pass: GitHub Actions run
...
this repository still double-applies on retry). Nothing was traded to hold the
...
| 5 | Payment idempotency | A retried payment request records and applies the payment **once**; on the processor-backed path it also authorizes once, and a same-key retry with different terms 409s | **Partial** | **Canonical, processor-backed path** — `payment-service/app/payments.py::charge`, `db/migrations/0007`, `0010`, `0012`, `0013`: required `idempotency_key`, partial unique index, `ON CONFLICT DO NOTHING`, apply-once via `payment_applications`, and `authorize_charge()` is not re-called on a retry. **Legacy processorless route** — `servicing-service/app/main.py::post_payment` → `payments.py::charge`: no key, no unique index, and `balance.apply_payment` called again on every retry | `tests/test_charge_flow.py`, `test_apply_payment_idempotency.py`; `test_reconcile_real_postgres.py` (**skips without PostgreSQL**; green in CI run `31759960436`). The legacy route's retry behaviour is now pinned by `servicing-service/tests/test_legacy_payments_is_not_idempotent.py`, which characterizes the defect rather than asserting it away | **G-D2-LEGACY** — servicing's own `POST /payments` **double-records the payment and double-applies the balance** on retry. It calls no processor, so no second authorization occurs and no settlement line exists for either row — which is also why reconciliation excludes them (D7). Bounded, not closed: it requires `X-Internal-Token` and the gateway 404s the path instead of proxying it, so it is reachable only from inside the compose network. *Read **Done** while a second `POST /payments` that double-records and double-applies was live.* | Medium | Retire the endpoint, or port `idempotency_key` to it. Deleting it is the smaller change and removes an unreconcilable capture path (D7) |
...
| 6 | Lost-update proof on the shared column | A real-PostgreSQL test pins concurrent payment behavior | **Done** | Ledger projection replaces the former unlocked payment read-modify-write path: `balance.py::apply_payment_once` inserts the entry, `db/migrations/0035_ledger_entries.sql`'s `project_ledger_entry()` composes the signed delta into `balances` | `servicing-service/tests/test_balance_lost_update_real_postgres.py` (5 cases) asserts both concurrent payments survive; `db/tests/test_0035_ledger_projection.py` (64 cases) proves parity. **Both skipped in this pass** — `DATABASE_URL` unset, no PostgreSQL available; the code path was read instead. Executed evidence is CI run `31759960436` on this exact commit, jobs `db-migrations` and `backend (servicing-service)` green | None for the lost update. Residual, distinct from D3: `balance.py::apply_payment` and `::waive_fee` return a figure computed before their own UPDATE, so the *returned* balance can be stale under concurrency while the stored one is correct | — | — |
...
already parametrized over **five** — the legacy `POST /payments` is guarded too.
...
- **G-D2-LEGACY** — servicing's own `POST /payments` accepts no idempotency key, so a retry **double-records the payment and double-applies the balance** (`DEBT.md` D2, open half). No processor is involved on this route, so nothing is authorized twice; what duplicates is the recorded payment and the balance movement, and because there is no processor there is no settlement line to corroborate either row — reconciliation excludes them by label (D7). Bounded by the internal token and by the gateway refusing to proxy the path. *AC:* the route is removed, or it requires a key and records and applies once. *Evidence:* a retry test on whichever survives; if removed, a test that the path is gone. The present behaviour is characterized by `servicing-service/tests/test_legacy_payments_is_not_idempotent.py`, which must be rewritten in the same change that closes this. *Dependency:* none.
...
legacy duplicate still double-applies on retry), **D8** (a validated human
...
- **Local run, and its one real limitation:** all eight backend suites plus
  `db/tests` passed; **no PostgreSQL was available**, so 489 real-database cases
  skipped across those suites and the Playwright specs did not run. Those rows
  cite CI run `31759960436` (head `c91fd19`, 2026-08-14T01:14Z, 23/23 jobs green)
  for their executed evidence instead of this pass.
