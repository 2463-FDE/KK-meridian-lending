...
| 3 | Explicit graph with per-step trace | Three nodes as a real `StateGraph` | **Done — with the trace half deliberately switched off** | `decision-service/app/graph.py:28,106`; suppression in `decision-service/app/tracing.py` | 40 tests green. **The evidence column used to read "LangSmith project `2463-fde`", which now points at the opposite of what happens**: the graph runs inside `suppressed_tracing()` and posts nothing, because the graph's state carries the applicant's SSN and ~30KB per decision was reaching a third-party SaaS. `decision-service/tests/test_the_decision_graph_transmits_nothing.py` | The graph is real and its nodes are explicit; per-step LangSmith visibility of a DECISION is not available and is not intended to be. The traced surface is the agent's underwriting summary path | — | — |
...
| 4 | Decisioning | The pull-credit/score/persist chain was a plain async function body — no explicit graph, no per-step trace | ✅ Fixed:<br>• `app/graph.py` — the exact same three steps (`_pull_credit` → `_run_model` → persist) as a real LangGraph `StateGraph`, not inline code<br>• `decide()`'s signature, output shape, and every fail-closed exception are unchanged — the graph's nodes call the identical functions decide() always called, so all 31 existing tests pass untouched<br>• LangSmith tracing came for free, and was then deliberately switched OFF: `langgraph` pulls in `langchain-core`, which auto-instruments when `LANGSMITH_TRACING`/`LANGSMITH_API_KEY` are set — already true via the shared `.env`. That is exactly the problem, because the graph's state carries the applicant's SSN, so `app/tracing.py` suppresses it and fails closed. This bullet used to end "no separate wiring needed", which read as a benefit and is now the record of how ~30KB per decision reached a third party without anyone wiring it up<br>• Live-verified: a real decision (app 7307, approve/672) ran through the graph end to end | Each step has an explicit node boundary; per-step LangSmith visibility of a DECISION is suppressed on purpose (`app/tracing.py`) and the suppressor fails closed. This column used to say each step "is now individually traceable" |
...
| # | Domain | What needed fixing | Fixed? | Why it mattered |
|---|---|---|---|---|
| 1 | Servicing | Any authenticated user, any role, could adjust any balance or waive any fee — no role check, no second approver | ✅ Fixed — three layers, landed in order: the gateway enforces staff-only server-side; servicing itself verifies WHO is acting from a gateway-signed assertion it cannot forge (`require_staff_principal`, PR #33); and `adjust-balance`/`waive-fee` now raise proposals that a *different* authorised person must approve, with `no_self_approval` refusing the same person on both halves (PRs #34/#35). **Bound:** any staff principal may propose against any serviced loan — there is no staff-to-loan assignment in this schema — and a direct database write bypasses the **application resolve path** — though not the proposal-matching trigger, which still refuses an `adjustment` or `fee_waived` entry without an approved, field-matching proposal by a distinct approver — since every service connects as the schema-owning role (ADR 0011, *Limitations*) | "Trusted reps" isn't a permissions model — a compromised or careless single account could zero out any loan with no second check |
...
**Amendment (2026-08-24), which changes what this week delivered.** The client
answered the protected-class question, and the answer removed a control rather
than unblocking one:
...
| 4 | KYC | CIP vs. CDD/ongoing-monitoring/SAR — different questions, not the same check done more thoroughly | 🟡 **Spec and ADR delivered; implementation not built; external inputs required.** `specs/0004-kyc-aml-ubo-and-sanctions-screening.md` and `adr/0012-sanctions-screening-integration.md` are on `main` and guarded by `db/tests/test_week9_kyc_aml_spec.py`. What is not built is the screening itself, and it is blocked rather than pending effort: four COMPLIANCE-BLOCKED decisions, two VENDOR-BLOCKED, three CLIENT-BLOCKED and one OPS-BLOCKED, listed under this table. `docs/DEBT.md` **D11** stays open: CIP-only onboarding is unchanged, `sanctions_screened` and `ubo_captured` are still hardcoded `False`, and no migration is written. *This cell read `⬜ Open (spec this week, not built)` until 2026-08-26 — the spec had existed since 2026-08-24, so the row asked for work that was already delivered.* | The compliance worksheet's "we check ID ✓" next to a blank "sanctions screen?" field is CIP mistaken for the whole program — exactly what "we're mostly there" gets wrong |
