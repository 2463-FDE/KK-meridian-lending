...
| 4 | KYC | CIP vs. CDD/ongoing-monitoring/SAR — different questions, not the same check done more thoroughly | 🟡 **Spec and ADR delivered; implementation not built; external inputs required.** `specs/0004-kyc-aml-ubo-and-sanctions-screening.md` and `adr/0012-sanctions-screening-integration.md` are on `main` and guarded by `db/tests/test_week9_kyc_aml_spec.py`. What is not built is the screening itself, and it is blocked rather than pending effort: four COMPLIANCE-BLOCKED decisions, two VENDOR-BLOCKED, three CLIENT-BLOCKED and one OPS-BLOCKED, listed under this table. *This cell read `⬜ Open (spec this week, not built)` until 2026-08-27 — the spec had existed since 2026-08-24, so the row asked for work that was already delivered.* | The compliance worksheet's "we check ID ✓" next to a blank "sanctions screen?" field is CIP mistaken for the whole program — exactly what "we're mostly there" gets wrong |
