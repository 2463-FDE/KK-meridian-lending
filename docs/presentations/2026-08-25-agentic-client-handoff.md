...
| **RF-25** manual DTI entry | **RULE ANSWERED 2026-08-29 -- DISPOSITIONED: DATA-MODEL EXPANSION REQUIRED. Nothing is built.** *This row read "OPEN CLIENT DECISION -- whether staff may apply DTI manually in a referred review, and what evidence authorises it" when this deck was written, and that was true then.* Both halves are answered: manual DTI is permitted only on a REFERRED application, only by an underwriter or admin, only from approved synthetic source documents, and it is human-review evidence that **must not change the decision**. It must persist gross monthly income, monthly debt obligations, source-document references, the calculation, staff identity, timestamp and reason -- a bare percentage is not sufficient. **The current data model cannot hold that truthfully**: there are no columns for the figures, there is no documents table anywhere for the references to point at, and `manual_reviews` requires an approve/deny outcome, so recording evidence there would itself decide the application. `docs/DEBT.md` RF-25 carries the traced gate and the smallest addition. Do not present this as a feature. | Client (rule, decided) / engineering (data-model expansion) |
