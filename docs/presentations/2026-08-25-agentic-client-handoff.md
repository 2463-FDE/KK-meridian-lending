...
total -- which can charge more than the decided rule allows, and which nothing on
that path stops being assessed twice. *When this deck was written the decided
rule needed installment-level facts the system did not persist. Since #143 it
persists them -- `ledger_entries.installment_no`, with one fee per installment
enforced by a database index -- and what remains is two answers rather than a
data-model change: the exact grace period, and the order in which a payment
settles overdue installments.* `docs/DEBT.md` D23 carries both. It is
deliberately not approximated from the past-due total.
...
| Item | Status | Owner |
|---|---|---|
| **D23** late-fee reassessment / compounding | **ANSWERED 2026-08-29 -- RULE DECIDED, NOT YET IMPLEMENTED.** *This row read "OPEN CLIENT DECISION -- may a fee be assessed again, at what cadence, and do previously assessed fees enter the next 5% base" when this document was written, and that was true then.* The answer replaces the rule rather than setting a cadence: at most one fee per missed scheduled installment, after the existing grace period, never reassessed against the same installment, priced at `min($35, 5% x unpaid scheduled P&I for THAT installment)` with all fees excluded from the base. **The code still does the older published comparison priced off the past-due total.** *This row also said the decided rule needs installment-level facts the system does not persist, and that was true when the deck was written. Since #143 (2026-08-31) it is not: `ledger_entries.installment_no` exists, one late fee per installment is enforced by a partial unique index, an installment outside the loan's schedule is refused by a trigger, and the decided arithmetic is implemented and tested.* What still blocks the runtime cutover is not a column. It is two answers nobody here holds: **the exact grace period** (recorded nowhere in the repository, including in the vendor's own fee schedule) and **the order in which a payment settles overdue installments** -- without which "unpaid scheduled P&I for that installment" is not derivable once any payment has been applied. `docs/DEBT.md` D23 carries what is built and what is waiting. Not approximated from `past_due`, deliberately. | Lending Operations (grace period + allocation order) / engineering (cutover, once answered) |
...
| **RF-25** manual DTI entry | **RULE ANSWERED 2026-08-29 -- AND BUILT SINCE, on 2026-08-31 (#146 schema, #151 API, #152 UI).** *This row read "OPEN CLIENT DECISION -- whether staff may apply DTI manually in a referred review, and what evidence authorises it" when this deck was written, and that was true then.* Both halves are answered: manual DTI is permitted only on a REFERRED application, only by an underwriter or admin, only from approved synthetic source documents, and it is human-review evidence that **must not change the decision**. It must persist gross monthly income, monthly debt obligations, source-document references, the calculation, staff identity, timestamp and reason -- a bare percentage is not sufficient. *This row said the current data model could not hold that truthfully -- no columns for the figures, no documents table for the references to point at, and `manual_reviews` requiring an approve/deny outcome so that recording evidence would itself decide the application. All three were true when this deck was written and were addressed on 2026-08-31.* `manual_dti_assessments` holds the figures with a CHECK tying the stored ratio to them, `manual_dti_source_documents` is the approved SYNTHETIC registry the references point at, and the evidence lives in its own append-only table so recording it decides nothing -- asserted against `decisions`, `applications.status`, `manual_reviews` and `decision_attempts` on both sides of a write. What remains open is a different question, tracked as **RF-29**: what an applicant may be TOLD when a human reviewer denies on a ratio. `docs/DEBT.md` RF-25 and RF-29 carry both halves. | Client (rule, decided) / engineering (delivered) |
...
- Not "production-ready", "fully secure", "PCI certified" or "SOC 2 compliant".
  **This is a synthetic training deployment.** Production hardening — password
  ...
  `SEC-01`..`SEC-18` in `docs/DEBT.md` and is **not**
