...
| D5a | Card/PII written to logs at INFO with no redaction. | **Fixed** — all three halves, re-verified against the code rather than the comments. `payment-service` redacts before the logger sees a payload (`redactor.py`). `servicing-service`'s legacy `POST /payments` no longer receives PAN/CVV/SSN at all — it takes an opaque `processor_token` plus `last4`/`brand` (ADR 0008), so there is nothing left to redact. Origination stopped logging the request payload (PR #6 review, Gap C); the intake line carries `app_id`/`applicant_id` only. **There is no request-body middleware in any service** — the gateway reads a body solely to forward it. *Previously recorded here as still open in servicing and origination: that was read off a stale docstring, not the code (see below).* | `origination-service/tests/test_intake_pii_not_logged.py`, `servicing-service/app/payments.py` |
| D5c | Four `logging_config.py` docstrings claimed *"Logs the full request body on every POST — including PII. No redaction."* about modules that only wire a stream and a file handler. The claim was copy-pasted across origination, decision, kyc and (in a charge-specific variant) servicing. | **Fixed** — docstrings corrected to describe the modules, with the old claim retained as history so the finding is not re-opened a third time. Recorded because the cost was real: this comment caused D5a to be reported as a live PII-logging gap twice, once in this register and once in a later documentation pass, both times by trusting a comment over the code. A comment that overstates a defect produces false findings as reliably as one that understates it. | `*/app/logging_config.py` |
