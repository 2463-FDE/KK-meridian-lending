...
| **D19** | `loans.apr` holds the **note rate**, not an APR — boarding writes the contractual rate there because servicing amortizes that column, and billing the disclosed APR would charge the borrower above their own disclosure (PR #10 review). The servicing UI still labels it "APR", so a funded loan displays 7.99% under a heading that should read 9.584%. The money is now right; the label is wrong. | **Open** — renaming the column touches servicing's API, UI and tests, and the disclosed APR would need carrying onto `loans` for display. Money correctness was fixed first, deliberately | `servicing-service/app/routers/loans.py`, `db/migrations/0030` |
