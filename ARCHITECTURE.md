...
| `payment-service` | 8006 | FastAPI + SQLAlchemy + psycopg2 | Card/ACH charge. Idempotency-key required, atomic dedupe + reconciliation (`db/migrations/0007`/`0012`/`0013`). Tokenizes card capture client-side (ADR 0008, supersedes ADR 0003) — never receives a raw PAN/CVV/SSN, only a processor token + last4/brand; the token itself is never persisted. After inserting the `payments` row it calls servicing's `apply-payment`. No host port; requires `X-Internal-Token`. |
