...
def test_the_readme_describes_both_assistant_routes_as_the_gateway_gates_them():
    ...
    routes with two gates. A truth-keeping change that replaces a stale claim
    ...
    staff-only". A skip is not a check: making the route staff-only would have
    silently turned this guard off and stopped verifying the README at all, on
    exactly the change that made its wording matter. It now asserts the
    staff-only contract positively, and asserts the OLD wording is gone.
    """
    ...
    # The route's gate decides which wording is true. Read from the code, so a
    # future loosening fails here instead of leaving the README to be believed.
    ...
    # The old claim must be gone, in every place it was made. `.` already
    # excludes newlines without DOTALL, so the window cannot run past the line.
    stale = re.findall(
        r"policy-chat.{0,120}(?:anonymous|open to any|without an account)",
        text, re.I)
    assert stale == [], (
        "README still calls policy-chat anonymous-allowed while the gateway "
        f"requires a staff session: {stale}")

    # And the new one must be present, near the route it describes -- in either
    # order, because the README states it both ways in different sections.
    says_staff_only = bool(
        re.search(r"policy-chat.{0,200}staff[- ]only", text, re.I | re.S)
        or re.search(r"staff[- ]only.{0,200}policy-chat", text, re.I | re.S))
    assert says_staff_only, (
        "README does not say /assistant/policy-chat is staff-only, so a reader "
        "carries the old anonymous claim across a route that now refuses them")
