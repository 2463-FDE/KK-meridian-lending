...
`main` today. Correct, so the change was split:
    ...
    0029  EXPAND    back-fill last4 from pan. Columns stay.   <- this release
    0031  CONTRACT  drop pan and cvv.                          <- the next one
...
This release is the expand half only, so these tests assert the overlap holds.
The contract half and its own tests ship separately, once this is deployed.
"""
import os
...
def test_expand_replays_cleanly(legacy_db):
    """The parity suite replays every migration twice, so the back-fill has to
    ...
    assert "pan" in _columns(legacy_db), "0029 must leave the contract step to 0031"
